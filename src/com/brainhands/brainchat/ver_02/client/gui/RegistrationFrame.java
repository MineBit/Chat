...
public class RegistrationFrame extends JFrame {
	...
	static void Close(){frame.setVisible(false);}

	//Класс, обработчик нажатия кнопки "Регистрация"
			...
			if(login.length() > 3){
				if(re_password.equals(password)){
					if (password.length() > 9) {
						if (ChatClient.Registration(login, password) == true){
							JOptionPane.showMessageDialog(null,"Регистрация успешно завершена!","Регистрация завершена!",JOptionPane.INFORMATION_MESSAGE);
							Close();
							StartFrame.View();
						}else{
							JOptionPane.showMessageDialog(null,"Данное имя пользователя уже занято!","Ошибка!",JOptionPane.ERROR_MESSAGE);
						}
					}else{
